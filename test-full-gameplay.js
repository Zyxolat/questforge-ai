...
async function runGameplayTest() {
  try {
    ...
    // Step 5b: Set chainQuestId in database for testing
    console.log('🔗 Step 5b: Setting chainQuestId for testing...');
    try {
      const cmd = `PGPASSWORD=postgres psql -U postgres -h localhost -d questforge -c "UPDATE \\"Quest\\" SET \\"chainQuestId\\" = 1 WHERE \\"id\\" = '${quest.id}';" 2>&1`;
      execSync(cmd);
      console.log(`✅ Quest chainQuestId set to 1\n`);
    } catch (e) {
      console.log(`⚠️  Could not set chainQuestId (may not be needed)\n`);
    }
